...
class JsonDeserializer() {
  ...
  def extractJsonToObject(jsonString: String): TwitterEvent = {
    ...
    val lang = jsonNode.get("lang").asText()
    val name = jsonNode.get("user").get("screen_name").asText()

    TwitterEvent(text = text, created_at = dateFormatter.parse(created_at), lang = lang, user = User(screen_name = name,profile_image_url = url))
